...
{
	i2c_start();
	i2c_write(SLAVE_WRITE);   //from which slave 
	i2c_write(address);    //some register address,hour,minute,sec
	i2c_write(data);       //get data from hour or second send it to this data
	i2c_stop();           //purge enable bit.
}
...
unsigned char read_ds1307(unsigned char address)
{
	...
	i2c_start();
	i2c_write(SLAVE_WRITE);           //informing rtc from which address you want to read the data
	i2c_write(address);              //
	i2c_rep_start();                //repeat start 
	i2c_write(SLAVE_READ);         //0xd1       
	data = i2c_read();
	i2c_stop();
